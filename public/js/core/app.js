        ...
        $.getJSON(assetPath + 'data/' + $filename + '.json', function (data) {
          for (var i = 0; i < data.listItems.length; i++) {
            ...
            if (data.listItems[i].name.toLowerCase().indexOf(value) == 0 && a < 5) {
              ...
              var id = data.listItems[i].id;

              var base_url = window.location.origin + '/admin/orders/show/' +id ; 

             /*  var url_order_show = "route('orders.show',':id')";

              url_order_show = url_order_show .replace(':id', id); */

              $startList += '<li class="auto-suggestion ' + $activeItemClass + '">' + '<a class="d-flex align-items-center justify-content-between w-100" href=' + base_url + '>' + '<div class="d-flex justify-content-start align-items-center">' + '<span>' + data.listItems[i].name +'-'+ data.listItems[i].status  + '</span>' + '</div>' + $bookmarkIcon + '</a>' + '</li>';
              a++;
            }
          }
